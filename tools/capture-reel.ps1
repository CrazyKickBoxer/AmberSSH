...
param([switch]$Capture, [switch]$Assemble, [switch]$SshOnly, [double]$Bpm = 90.5, [double]$FirstBeat = 0.35,
      [string]$Profile = "", [double]$Beats = 90.0,
...
if ($Assemble) {
  ...
  if ($SshOnly) {
    ...
    $cards = @(
      # the desktop, first
      (Card "A LINUX DESKTOP" 0.6 2.4 82 $orb 0.09),
      (Card "drawn in particles, live over VNC" 1.1 1.9 38 $mic 0.16),
      (Card "THE MENU, KEY BY KEY" 3.0 3.2 56 $orb 0.09),
      (Card "every arrow is a real key on the far side" 3.6 2.6 30 $mic 0.15),
      (Card "A FILE MANAGER" 11.4 3.2 56 $orb 0.09),
      (Card "opened, driven, and closed again" 12.0 2.6 30 $mic 0.15),
      (Card "REAL INPUT" 18.6 2.4 60 $orb 0.09),
      (Card "commands running on the far side" 19.1 1.9 34 $mic 0.15),
      (Card "IRIS" 20.6 1.5 50 $orb 0.09),
      (Card "SHATTER" 25.0 1.5 50 $orb 0.09),
      (Card "SHEAR PLATES" 29.4 1.5 50 $orb 0.09),
      (Card "LIGHT SPEED" 31.6 1.5 50 $orb 0.09),
      (Card "BURN" 36.0 1.5 50 $orb 0.09),
      # the terminal
      (Card "AND A TERMINAL" 38.65 1.9 86 $orb 0.40),
      (Card "made of the same particles" 39.1 1.4 36 $mic 0.50),
      (Card "2000 GLYPHS, ONE FRAME" 40.4 2.3 58 $orb 0.09),
      (Card "a real directory, re-forming on every half beat" 40.9 1.8 30 $mic 0.15),
      (Card "DIGITAL RAIN" 44.1 2.4 54 $orb 0.09),
      (Card "ls /etc" 44.5 1.9 28 $mono 0.155),
      (Card "SONIC BOOM" 46.75 2.4 54 $orb 0.09),
      (Card "ls /usr/lib64" 47.15 1.9 28 $mono 0.155),
      (Card "MAGNETIC ASSEMBLE" 49.4 2.4 54 $orb 0.09),
      (Card "ls -lhA /var/log" 49.8 1.9 28 $mono 0.155),
      (Card "CYCLONE" 52.05 2.4 54 $orb 0.09),
      (Card "sftp - 28 files, every bar moving" 52.45 1.9 28 $mono 0.155),
      (Card "GLITCH" 54.7 2.4 54 $orb 0.09),
      (Card "ls /usr/share" 55.1 1.9 28 $mono 0.155),
      (Card "STARWAKE" 57.35 2.4 54 $orb 0.09),
      (Card "ls /usr/bin" 57.75 1.9 28 $mono 0.155),
      (Card "FILM BURN" 60.0 2.4 54 $orb 0.09),
      (Card "rsync - incremental, mid-flight" 60.4 1.9 28 $mono 0.155),
      (Card "MURMURATION" 62.65 2.4 54 $orb 0.09),
      (Card "HAMMER" 65.3 2.4 54 $orb 0.09),
      (Card "15 INTERFACE STYLES" 68.8 3.6 66 $orb 0.09),
      (Card "one run through the set" 69.3 3 30 $mic 0.175),
      (Card "4 APPEARANCES" 77.6 2.6 56 $orb 0.09),
      (Card "AMBERSSH" 82.2 6.4 150 $orb 0.42),
      (Card "ssh   telnet   serial   local   vnc" 83.4 5.2 40 $mono 0.56)
    )
  }
  $fadeOut = $length - 1.6
