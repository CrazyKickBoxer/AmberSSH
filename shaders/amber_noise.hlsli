// amber_noise.hlsli — the pure helpers shared by every particle shader:
// an integer hash, 3D simplex noise, and its 2D curl. Split out of
// amber_common.hlsli so that shaders which bind their own constant buffer
// (the VNC desktop, at b1) can use them without seeing FrameCB — HLSL
// constant-buffer members are globals, and two buffers naming `time`
// cannot coexist in one translation unit.
#ifndef AMBER_NOISE_HLSLI
#define AMBER_NOISE_HLSLI
