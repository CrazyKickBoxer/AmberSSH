...
void App::DrawVncScene(ID3D12GraphicsCommandList* cl, FrameContext& frame, D3D12_GPU_VIRTUAL_ADDRESS cb)
{
...
// ---- --vnc-selfcheck ------------------------------------------------------------
// The faithful contract, measured. An RFB server in-process serves a known
// picture; a VNC tab at solidity 1, density 1, size 1 draws it; the scene
// target — before bloom and composite, in linear light — is read back and
// every pixel compared with the decoded colour, to within one 8-bit step
// after the sRGB curve. Then the server recolours one block: the picture
// must follow it and nothing else may move, and the disturbance energy
// texture must be non-zero inside the block and zero outside. A key and a
// pointer event are sent and counted on the far side. The report goes to
// %TEMP%\vnc-selfcheck.txt and the exit code says pass or fail.
//
// What this does NOT check, and says so: the composite stage (the scene
// is compared, not the swap chain), scaled placement, and anything a real
// server does differently from this one.
...
struct App::VncSelfCheck
{
    ...
    int phase = 0;              // 0 start, 1 first picture, 2 read, 3 after change, 4 read, 5 done
    int framesInPhase = 0;
    ...
    {
        char line[640];
        snprintf(line, sizeof line, "%-64s %s  %s\n", what, ok ? "ok  " : "FAIL", detail.c_str());
        report += line;
        if (!ok)
            ++failures;
    }
...
void App::VncSelfCheckTick()
{
    if (!m_vncCheck)
    {
        ...
        c.startedAt = m_time;
        const bool up = c.server.Start(VncSelfCheck::kW, VncSelfCheck::kH);
        c.Check("in-process RFB server started", up, up ? "127.0.0.1:" + std::to_string(c.server.Port()) : "bind failed");
        if (!up)
        {
            c.phase = 5;
            return;
        }
        m_appearance = 0;   // the dark ground; the picture covers itself anyway
        amber::ConnectionRequest req;
        req.profile.id = amber::MakeUuid();
        req.profile.name = "vnc self-check";
    ...
    if (c.phase < 5 && m_time - c.startedAt > 25.0)
    {
        c.Check("the self-check completed within 25 s", false,
    ...
    {
    case 1:
        if (s->state == amber::SessionState::Connected && t->updatesSeen >= 1 && t->desk && t->desk->Ready())
        {
            if (++c.framesInPhase >= 30)
            {
                ...
                c.phase = 2;   // the next frame's end reads the scene
            }
        }
        break;
    case 3:
    ...
    case 5:
    {
        ...
        char buf[256];
        snprintf(buf, sizeof buf, "layout: particles %u, density %u, stride %u, sampled %ux%u, gpu bytes %.1f MiB",
                 L.particles, L.density, L.stride, L.sampledW, L.sampledH,
                 static_cast<double>(L.gpuBytes) / (1024.0 * 1024.0));
        c.Info(buf);
        snprintf(buf, sizeof buf, "gpu (last frame): desktop upload %.3f ms, sim %.3f ms, draw %.3f ms, frame %.3f ms",
                 m_device.GpuDesktopUploadMs(), m_device.GpuDesktopSimMs(), m_device.GpuDesktopDrawMs(),
                 m_device.GpuFrameMs());
        c.Info(buf);
        snprintf(buf, sizeof buf, "window %ux%u, video memory %.0f MiB", m_device.Width(), m_device.Height(),
                 static_cast<double>(m_device.VideoMemoryBytes()) / (1024.0 * 1024.0));
        c.Info(buf);
        c.report += c.failures ? "\nSELFCHECK FAILED\n" : "\nSELFCHECK PASSED\n";
        wchar_t tmp[MAX_PATH];
        GetTempPathW(MAX_PATH, tmp);
        const std::wstring path = std::wstring(tmp) + L"vnc-selfcheck.txt";
        if (FILE* f = _wfopen(path.c_str(), L"wb"))
        {
            fwrite(c.report.data(), 1, c.report.size(), f);
            fclose(f);
        }
        const int code = c.failures ? 1 : 0;
        c.server.Stop();
        delete m_vncCheck;
        m_vncCheck = nullptr;
...
void App::VncSelfCheckAfterFrame()
{
    ...
    c.Check(after ? "scene target read back after the change" : "scene target read back", ok);
    if (ok)
    {
        if (!after)
        ...
        int mismatches = 0, maxDev = 0, inBlockMismatch = 0;
        long long checked = 0;
        std::string first;
        for (uint32_t y = 0; y < VncSelfCheck::kH; ++y)
            for (uint32_t x = 0; x < VncSelfCheck::kW; ++x)
            {
                const uint32_t e = c.server.PixelAt(x, y);
                ...
                {
                    ++mismatches;
                    const bool inBlock = x >= VncSelfCheck::kBlockX && x < VncSelfCheck::kBlockX + VncSelfCheck::kBlockW &&
                    ...
                    if (first.empty())
                        first = "first at (" + std::to_string(x) + "," + std::to_string(y) + "): got " +
                                ...
                                " expected " + std::to_string(er) + "," + std::to_string(eg) + "," + std::to_string(eb);
                }
            }
        char d[320];
        snprintf(d, sizeof d, "%lld pixels, %d off by more than 1/255, max deviation %d%s%s", checked, mismatches,
                 maxDev, first.empty() ? "" : "; ", first.c_str());
        c.Check(after ? "the changed block shows its new colour and nothing else moved"
                      : "solidity 1 at native scale reproduces every decoded pixel (within 1/255)",
                mismatches == 0, d);
        if (after)
            c.Info("of which inside the changed block: " + std::to_string(inBlockMismatch));
    }

    if (!after)
    {
        // the second half: input out, a block changed
        ...
        for (uint32_t y = 0; y < eh; ++y)
            for (uint32_t x = 0; x < ew; ++x)
            {
                ...
                const bool in = x >= VncSelfCheck::kBlockX && x < VncSelfCheck::kBlockX + VncSelfCheck::kBlockW &&
                                y >= VncSelfCheck::kBlockY && y < VncSelfCheck::kBlockY + VncSelfCheck::kBlockH;
                (in ? inside : outside) = std::max(in ? inside : outside, v);
            }
        char d[128];
    ...
    c.Check("one connection served the whole check", c.server.Accepted() == 1,
            "accepted " + std::to_string(c.server.Accepted()) + ", updates " + std::to_string(c.server.UpdatesSent()));
    c.phase = 5;
}
